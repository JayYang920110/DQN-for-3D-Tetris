...
class Tetris3D:
    ...
    def __init__(self, height: int = 20, width: int = 4, depth: int = 4, device: str = "cpu"):
        ...
        self.device = device
        self.piece_colors = ['deepskyblue', 'limegreen', 'orangered', 'khaki']  # 顏色對應到方塊類型
    ...
    def step(self, action, render=False, frames=None):
        """Place the block at (rotation_idx, x, y). Return (reward, done)."""
        if self.gameover:
                    ...
                    for dx in range(px):
                        for dy in range(py):
                            if self.current_piece[dz, dx, dy]:
                                ...
                                yy = y + dy
                                if 0 <= zz < self.height:
                                    temp_board[zz, xx, yy] = self.current_piece_type + 1
            ...
            for dx in range(px):
                for dy in range(py):
                    if piece[dz,dx,dy]:
                        ...
                        x = pos["x"] + dx
                        y = pos["y"] + dy
                        self.board[z, x, y] = self.current_piece_type + 1
...
def render_voxel_video_matplotlib(board_sequence, piece_colors, save_path="tetris3d_total_reward.mp4", fps=5, frame_rewards=None):
    ...
    # 預先計算 cumulative rewards
    cumulative_rewards = []
    total = 0.0
    for r in frame_rewards or []:
        total += r
        cumulative_rewards.append(total)

    def draw_frame(board, total_reward):
        ax.clear()
        fig.texts.clear() # 清除上一幀的文字    
        ...
        # ✅ 顯示累積 Total Reward（使用 fig.text 避免被方塊擋住）
        fig.text(0.02, 0.92, f"Total Reward: {total_reward:.2f}", fontsize=13, color='darkred')

        # 畫出所有方塊
        z, x, y = np.where(board > 0)
        for xi, yi, zi in zip(x, y, z):
            block_id = board[zi, xi, yi]
            color = piece_colors[block_id - 1]
            draw_cube(ax, xi, yi, zi, color=color)
    ...
    for i, board in enumerate(board_sequence):
        total_r = cumulative_rewards[i] if frame_rewards and i < len(cumulative_rewards) else 0.0
        images.append(draw_frame(board, total_r))
        fig.texts.clear()  # 清除上一幀的文字
